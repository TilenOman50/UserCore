...
const STEP_META: Record<SubStepType, { label: string; icon: ReactElement }> = {
  "terms-acceptance": { label: "Terms & conditions", icon: <FileIcon /> },
  "email-verification": { label: "Email verification", icon: <EmailIcon /> },
  "id-scan": { label: "Identity document", icon: <IdIcon /> },
  "face-scan": { label: "Face scan", icon: <FaceIcon /> },
  "proof-of-residence": { label: "Proof of residence", icon: <HouseIcon /> },
  "contact-information": { label: "Contact information", icon: <PhoneIcon /> },
};
...
export const OverviewScreen = ({
  ...
  if (showQR && mobileUrl) {
    return <QRView mobileUrl={mobileUrl} onBack={() => setShowQR(false)} />;
  }
  ...
  const primaryLabel = allDone ? "Submit for review" : "Continue";
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {s.current && !s.completed && (
                  <div className="text-xs text-primary-700 mt-0.5">Up next</div>
                )}
              </div>
            </li>
          );
...
const QRView = ({
  ...
  <div className="flex flex-col h-full">
    ...
    <p className="text-sm text-gray-500 mt-1">
      Scan the QR with your phone camera. The flow will pick up exactly where
      you left off.
    </p>
    ...
    <div className="mb-3">
      <label className="text-xs font-medium text-gray-600">Direct URL</label>
