// In-process tms-api harness. The audit-log routes are internal to the
// platform — written by event subscribers, read by the dashboard. The
// RabbitMQ client is stubbed with a fake that records publishes and lets
// a test invoke registered handlers directly, the way a real broker would.
//
// PGLite is instantiated here (not through createDB) so we can call
...
import { createTmsApi } from "../../src/tmsApi";
...
export type TestApp = {
  app: ReturnType<typeof createTmsApi>;
  db: Database;
  rabbit: FakeRabbit;
...
export const bootTestApp = async (): Promise<TestApp> => {
  ...
  const app = createTmsApi({ db, logger, rabbitMQ: rabbit });
